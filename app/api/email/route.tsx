...
export async function POST(request: NextRequest) {
  ...
  // start-------------------------------------------問い合わせ者様向けメール内容
  const mailOptionsCustomer: Mail.Options = {
    from: process.env.NEXT_PUBLIC_MAIL_ADDRESS,
    to: `${data.email}`,
    subject: `【エムパッケージ】お問い合わせ完了`,
    html: `
    <head>
                      ...
                      <tr>
                        <td style="background-color: #ffffff; padding: 30px 0;">
                          <table ellspacing="0" cellpadding="0" border="0" width="90%" style="background-color: #ffffff; margin: auto;">
                            ...
                            <tr>
                              <td style="text-align: center; border:1px solid #555555; padding: 15px; font-family: sans-serif; font-size: 10px; font-weight: bold; color: #555555;">
                                <span style="margin: 0;">※本メールはシステムにより自動送信しております。</span>
                              </td>
                            </tr>
  ...
  //   transporter.sendMail(mailOptionsAdmin, function (err) {
  //     if (!err) {
  //       resolve('メールが送信されました。');
  //     } else {
  //       reject(err.message);
  ...
  try {
    ...
    // await Promise.all([transporter.sendMail(mailOptionsAdmin), transporter.sendMail(mailOptionsCustomer)])
    // .then((respose) => {
    //   console.log(respose)
    // })
    // .catch((error) => {
